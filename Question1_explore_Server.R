Question1_explore_Server <- function(input, output, session) {
  ...
  output$explore_1 <- renderGirafe({
    ...
    sailor_vertex_name <- mc1_nodes_clean %>%
      filter(node_name == input$artist_1) %>%
      pull(name)
    ...
    sailor_music_all <- mc1_nodes_clean %>%
      filter(name %in% sailor_out_node_names, 
             `Node Type` %in% input$node_type_filter_e_1) %>%
      pull(name)
    ...
    filtered_edges <- sub_graph %>%
      activate(edges) %>%
      filter(`Edge Colour` %in% input$edge_type_filter_e_1)
    ...
    g <- filtered_edges %>%
      ...
      geom_point_interactive(
        aes(
          ...
          colour = genre,
          shape = `Node Type`,
          size = ifelse(node_name == input$artist_1, 3, 1),
          tooltip = case_when(
      ...
      geom_node_text(
        aes(
          label = ifelse(node_name == input$artist_1, input$artist_1, NA)
        ),
        fontface = "bold",
        size = 2.5,
        colour = 'red',
    ...
    g <- filtered_graph %>%
      ...
      geom_point_interactive(
        aes(
          ...
          colour = genre,
          shape = `Node Type`,
          size = ifelse(node_name == input$artist_2, 3, 1),
          tooltip = case_when(
            `Node Type` == "Album" ~ sprintf(
              "%s %s \nNotable: %s (%s)", node_name, genre, notable, release_date
            ),
      ...
      geom_node_text(
        aes(
          label = ifelse(node_name == input$artist_2, input$artist_2, NA)
        ),
        fontface = "bold",
        size = 2.5,
        colour = 'red',
  ...
  output$explore_3 <- renderGirafe({
    # Data Preparation
    # Step 1: Get the node of the sailor
    sailor_node <- creator_and_songs_and_influenced_by_creator %>%
      filter(creator_name == input$artist_3) %>%
      pull(creator_from) %>%
      unique()
    ...
    g <- filtered_graph %>%
      ...
      geom_node_text(
        aes(
          label = ifelse(node_name == input$artist_3, input$artist_3, NA)
        ),
        fontface = "bold",
        size = 2.5,
        colour = 'red',
